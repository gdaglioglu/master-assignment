 ...
 */
public class DatabaseLocationDialog extends WindowAdapter implements
		ActionListener, Observer {
	/*
	 * The strings for the title and buttons in the dialog box. While these
	 * could be hard coded in the application, having them here makes it easier
	 * to use internationalisation options such as a ResourceBundle.
	 */
	...
	public DatabaseLocationDialog(ApplicationMode connectionMode) {
		...
		options = new JOptionPane(configOptions, JOptionPane.QUESTION_MESSAGE,
	 ...
	 * buttons being clicked) and undesirable actions (the window being closed)
	 ...
	 * (even though we only have three modes), so it does not have any knowledge
	 * of how we are using it within this dialog box. So ConfigOptions just
	 * sends updates to registered Observers whenever anything changes. We can
	 * receive those notifications here, and decide whether we have enough
	 * information to enable the "Connect" button of the dialog box.
	 */
	@Override
	...
	/**
	 * Returns the type of network connection (DIRECT, RMI ...) that should be
